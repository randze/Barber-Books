...
function UserScheduler(props) {
	...
	const selectDateTime = (value) => {
		setFormObject({ ...formObject, time: value.format(format) })
	}
	...
	function disabledTime(date) {
		...
		if (date && [0, 6].includes(date.day())) {
			...
			takenHours = [
				...takenHours,
				...[...Array(10).keys()],
				...[...Array(4).keys()].map((item) => 23 - item),
			]
		} else {
			takenHours = [
				...takenHours,
				...[...Array(8).keys()],
				...[...Array(2).keys()].map((item) => 23 - item),
			]
		}
		...
		return {
			...
			disabledMinutes(h) {
				for (let i = 0; i < unavailDateTime.length; i++) {
					if (
						date.year() === unavailDateTime[i].year &&
						date.month() === unavailDateTime[i].month &&
						date.date() === unavailDateTime[i].date &&
						h === unavailDateTime[i].hour
					) {
